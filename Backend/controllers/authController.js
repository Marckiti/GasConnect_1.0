...
const registerUser = async (req, res) => {
    ...
    try {
        ...
        // ── Plantilla: Confirmación de Correo ──────────────────────────────────────────
        const contenidoConfirmacion = `
          ...
          <p style="margin:0 0 20px; color:#555555; font-size:15px; line-height:1.6;">
            ...
            Por favor, confirma tu dirección de correo electrónico para completar
            el registro y acceder a todos los servicios.
          </p>
          ...
          <div style="text-align:center; margin:30px 0;">
            <a href="${confirmationUrl}" style="display:inline-block; background: linear-gradient(135deg,#ff6600,#ff8c00);
          ...
          <p style="margin:24px 0; color:#555555; font-size:14px; line-height:1.6;">
            O copia y pega este enlace en tu navegador:
          </p>
          <p style="margin:0 0 20px; padding:12px; background:#f5f5f5; border-radius:6px; 
                    font-size:12px; color:#666; word-break:break-all; border-left:3px solid #ff6600;">
            ${confirmationUrl}
          </p>
          ...
          <p style="margin:24px 0 0; font-size:13px; color:#aaaaaa; text-align:center;">
            GasConnect - Tu distribuidora de gas en Quito
          </p>
        `;
        ...
        try {
            await sendEmail({
                email: user.email,
                subject: '¡Confirma tu correo en GasConnect! 🔐',
                html: emailHtml
            });
        } catch (err) {
            console.error('Error enviando correo de confirmación:', err.message);
            // No retornar error, permitir que continúe
        }
    ...
    try {
        ...
        if (!user) {
            return res.status(400).json({ 
                ...
                debug: { tokenRecibido: cleanToken, tokenHasheado: hashedToken }
            });
        }
...
module.exports = { registerUser, loginUser, profileUser, forgotPassword, resetPassword, confirmEmail };
